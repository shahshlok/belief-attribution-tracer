...
pytestmark = pytest.mark.skipif(
    os.getenv("RUN_PROVIDER_TESTS") != "1" or not os.getenv("ANTHROPIC_API_KEY"),
    reason="Provider smoke tests require RUN_PROVIDER_TESTS=1 and ANTHROPIC_API_KEY.",
)
...
async def test_get_structured_response_real():
    """Real test for standard structured response using Claude Haiku."""
    result = await anthropic_module.get_structured_response(
        messages=[
            {
                "role": "user",
...
async def test_get_reasoning_response_real():
    """Real test for reasoning response using Claude Haiku with extended thinking."""
    result = await anthropic_module.get_reasoning_response(
        messages=[
            {
                "role": "user",
...
async def test_system_message_handling():
    """Test that system messages are properly extracted and passed to Anthropic."""
    result = await anthropic_module.get_structured_response(
        messages=[
            {
                "role": "system",
